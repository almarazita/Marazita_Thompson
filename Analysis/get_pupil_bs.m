function get_pupil_bs(all_pyr_cleaned_data, will_save)
%% Plots the coefficient from the linear regression of baseline-subtracted
% evoked against baseline pupil across different evoked time windows.

% Default to not saving each session's figure
if(nargin < 2)
    will_save = false;
end
...
for i = 1:num_sessions
    ...
    for tr = 1:num_trials
        ...
        sample_on_idx = 301;
        sac_on_idx = cur_session.times(tr, 18); % sac_on = column 18
        sac_on_idx = sac_on_idx{1,1}*1000;
        sac_on_idx = round(sac_on_idx);
    ...
    %% Plot
    fig = figure;
    xaxis = (0:(max_common_windows - 1)) * step_size;
    ...
    if will_save
        pdfFileName = sessionName+"_Betas.pdf";
...
% Optionally, save as PDF
if will_save
    pdfFileName = "All_Sessions_Betas.pdf";
    exportgraphics(fig, pdfFileName, 'ContentType', 'vector');
    close(fig);
end