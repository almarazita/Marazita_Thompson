function new_plotSwitchPSTH(data,axs)
...
prevState = [nan; data.ids.correct_target(1:end-1)]; % What the previous correct target was
new_switch_bool = zeros(length(choice),1);
new_switch_bool(choice~=prevState)=1; % The monkey switched if its current choice is not what was previously correct
new_switch_bool(~ismember(choice, [1 2]) | ...
    ~ismember(prevState,[1 2])) = nan; % If the previous state or current choice are NaN, then so is switch

switch_bool = new_switch_bool;
...
if ~sum(data.values.hazard == 0.05)
    ...
    for u = 1:n_units
        ...
        for c = 1:length(codes)
            ...
            for s = 1:2
                ...
                p(s) = new_plotPSTHAligned(tmp,event_code,window_width,ax,co{s},1);
            end
            if isempty(axs)
